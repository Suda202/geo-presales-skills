...
| `geo-presales-report-audit` | 需要审核或修正品牌提及识别与正文首次出现排序 | 修正后的 `brand_rankings`、安全补丁、可复现的问题说明与必要的 Bad Case 草稿 | 不修改客户报告分析结论，不审计或修改情绪，不做竞品研究或出题 |

先用 `geo-presales-report-audit` 修正品牌提及识别与首次出现排序等底层结果；再用 `geo-presales-report-editor` 基于已确认的结果修改分析结论并生成上传 CSV。回答级情绪审计已暂停，待句子级方案和验收标准确认后再恢复。两者处理报告的不同层次，可以连续使用。
