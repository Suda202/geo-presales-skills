...
def compute_metrics(config: dict, question_bank: dict, answers_doc: dict) -> dict:
    ...
    generic = [item for item in selected_valid if item["question_type"] == "generic"]
    ...
    for obj in config["objects"]:
        included = [answer for answer in generic if _object(answer, obj["object_id"])["mentioned"]]
        mention_counts[obj["object_id"]] = len(included)
        mention_values[obj["object_id"]] = _percent(len(included), len(generic))
    ...
    mentioned_target = [answer for answer in selected_valid if _object(answer, target_id)["mentioned"]]
    resolved_sentiments = [
    ...
    raw_citations = [citation for answer in selected_valid for citation in answer["citations"]]
    ...
    priority_share = opportunity_counts["priority_improve"] / len(generic) if generic else None
    ...
    metrics = {
        ...
        "coverage": {
            ...
            "valid_generic_answers": len(generic),
        },
        "overview": {
        ...
        "methodology": {
            ...
            "share_of_voice_denominator": "有效通用回答中各已配置对象提及次数之和",
            "positive_sentiment_denominator": "已完成情绪判断的监测对象正向与负向提及之和；排除中性",
            "official_citation_denominator": "所有入选有效回答产生的原始引用记录",
            "source_type_denominator": "按 answer_id 与 canonical_url 组合去重后的引用",
