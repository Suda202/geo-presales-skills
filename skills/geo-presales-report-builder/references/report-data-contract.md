...
```json
{
  ...
  "sources": {
    ...
    "domains": [["fragrance.com","31.0%","社交平台"]],
    "third_party_domains": [["fragrance.com","31.0%","社交平台"]],
...
- `domains` 每行 3 项：`[域名, 引用份额字符串, 类别中文名]`，按未去重引用次数降序，最多 8 行。
- `third_party_domains` 同结构，但只收非「自有网站 / 竞品网站」的来源、最多 5 行、不受 top8 截断——供内容规划的「第三方阵地」用，避免 top8 被官网与竞品占满后该面板为空。
...
- `theme_matrix` 是**主题 × 品牌**矩阵（`themes` / `brands` / `matrix[theme][brand]`），与 `matrix` 并存的另一维度，别混用：
  - 单元格取该主题下**计数最高的 attribute** 作为代表描述（`top_attribute` + `top_dir`），前端展示的是这个描述文本，**不是数字**；`pos`/`neg`/`rate` 只供筛选与排序。展示 attribute（而非 claim）是因为**极性住在 attribute 里**（`价格水平 > 有竞争力`），只显示 claim 会丢掉正负；且 attribute 跨品牌通用，正是拿来横向比较的层级。字段名 2026-09-21 由 `top_claim` 更正为 `top_attribute`——Claim 层分层后该格放的是 attribute，句级降级路径放的仍是观点标签，两者同格同义。
