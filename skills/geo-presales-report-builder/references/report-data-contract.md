...
- `theme_matrix` 是**主题 × 品牌**矩阵（`themes` / `brands` / `matrix[theme][brand]`），与 `matrix` 并存的另一维度，别混用：
  - 单元格取该主题下**计数最高的 claim 标签**作为代表描述（`top_claim` + `top_dir`），前端展示的是这个描述文本，**不是数字**；`pos`/`neg`/`rate` 只供筛选与排序。字段名是 `top_claim`：Claim 路径早期误写成 `top_attribute`，而前端只读 `top_claim`，导致矩阵整块显示「—」（2026-09-21 修正）。
