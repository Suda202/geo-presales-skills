...
def _module_instruction(module_id: str) -> str:
    instructions = {
        "M01": "用中文解释确定性数据总览和采集覆盖情况。禁止把本批次观察外推为整体市场结论。",
        "M02": "用中文解释监测对象与竞品的提及率、声量占比和首次出现顺序。禁止把首次出现顺序称为语义推荐排名。",
...
def publish_checks(config: dict, question_bank: dict, answers_doc: dict, metrics: dict, manifest: dict) -> list[dict]:
    ...
    generic_count = metrics["coverage"]["valid_generic_answers"]
    bucket_count = sum(metrics["opportunity"]["counts"].values())
    add("OPPORTUNITY_PARTITION", bucket_count == generic_count, "blocking", "每条有效通用回答恰好进入一个机会档位")
...
def finalize_report(root: Path, manifest: dict, config: dict, question_bank: dict, answers_doc: dict, metrics: dict) -> tuple[dict, dict]:
    ...
    report = {
        ...
        "scope": {
            ...
            "disclaimer": "本报告基于一个 Topic、固定问题集和本批次 ChatGPT 回答，用于观察该范围内的品牌与竞品表现，不代表全部用户需求、搜索量或市场份额。",
        },
        "metrics": metrics,
        "modules": {
    ...
    audit = {
        ...
        "known_limitations": [
            "report_rank 表示已配置对象的首次出现顺序，不是语义推荐排名",
