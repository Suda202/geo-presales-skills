...
- **语义环节**:逐单元读,拆出 Claim 并给 Attribute / Theme / 方向;同回答内语义相同的观点归一(跨回答保留)。不判事实真假;品牌未提及或无明确倾向的不生成 Claim,不记中性、不补值。
  - **分片抽取时先冻结主题词(2026-09-21 教训)**:单元多时会把语义环节派给多个并行子代理,若不先给一份统一的 Theme 词汇表,各片会各自发明主题——实测同一个 Trip.Biz 报告里「服务与售后」和「服务与覆盖」并存、还漏进净水器品类的「安装与部署」。做法:抽取前先按品类定一份主题清单,交给每个子代理;**合并后校验主题集合**是否收敛,近义主题(如「服务与覆盖」→「服务与售后」)要归一后再进矩阵。Attribute 同理,但不能靠关键词自动打标,归一由人/模型判。
- `claims-assemble`(确定性):校验并回装上下文——`unit_index` 必须指向真实单元、品牌必须与单元一致、方向只允许正/负。产出 `claims.json`。
- `claims-metrics`(确定性):按下方口径出指标。

## 统计口径(不得自行变通)

- **回答内去重**:同一回答内,同一品牌 × 同一 **semantic claim 只计 1 次**;同一 Attribute 下的**不同 Claim 各计一次**;**跨回答分别计数**。语义相同的 Claim 由语义环节归一,`claims-metrics` 再按 claim 文本(压空白 + 忽略大小写)做确定性兜底去重。
...
| 同一 Attribute 有多个近义 Claim | 语义相同的在语义环节归一成一条;确实不同的 Claim 明细全部保留,统计各计一次 |
