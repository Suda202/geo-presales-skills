...
每个 Case 对应一条 Record，且每条 Record 只表达一条产品线。同一公司的不同产品线必须分别创建 Record，不得通过增加 Topic 合并。写入前先用 `field-list` 获取真实 schema，再读取 Records，以 `品牌名称 + 业务 / 产品名称` 识别已有产品线 Case，用“主题”核对该 Record 是否包含恰好两个同产品线 Topic。维护同一 Case 时更新原 Record；新品牌或新产品线创建 Record。
