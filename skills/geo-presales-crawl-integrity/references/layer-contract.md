...
|---|---|---|---|---|---|
| `chatgpt` | `task_result.result_text` | 额外字段，现阶段自己识别 | `content_references` | `search_result`、`links`、`sse_data` | 分开展示两个字段：`content_references`（引用）、`search_result`（来源） |
...
- **来源面板取数：AI Mode 与 Gemini 取 `citations`，AIO 取 `source`**；**ChatGPT 没有单装来源面板的字段，右侧分开展示两个字段：`content_references`（引用）与 `search_result`（来源）**。
- **取数时按用途区分，不因字段相同互相替代。**
