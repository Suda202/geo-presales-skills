...
3. 按目标用户、评估标准、约束和比较对象建立意图矩阵；条件可单用或复合，但不得拼接任务。Generic 不写任何具体品牌实体，但题目本身必须明确要求品牌/供应商/平台/产品候选、候选间比较，或最终候选选择，不能依赖 AI 在回答材料、标准或品类问题时偶然顺带提品牌。答案候选可包含监控品牌、已配置竞品或未配置的同类品牌：要候选是 Recommendation，要候选间差异与取舍是 Comparison，要最终选择具体候选是 Decision。v3 不使用 `geo_intent`；价格、风险、替代和试用只是改变选择的条件。
4. 一题一意图：条件取值不同，或增删会改变答案的条件，才算不同意图；条件相同、只换措辞或顺序不算。句式相同与 Yes/No 不直接判错。
5. Generic 不得出现客户品牌、产品 aliases、竞品或其他任何具体品牌实体；程序对配置实体做硬拦截，未配置实体由语义 review 拦截。每 Topic 在 Branded 中固定生成 1 条 `Evaluate the {品类} company/product {品牌} on {主题}` 品牌总体评价基准题，硬归 `branded + decision`，由模板本身识别，不新增情绪意图标签。这不是唯一情绪样本：其余 Branded 题继续探测具体人群、能力、竞品和选择下的品牌评价。Branded Comparison 达到 5 条时采用三竞品各一条，加两条关键因素或多品牌题；不越过低可比竞品的 `allowed_dimensions`。
...
7. 二遍逐题确认购买对象仍属于 Topic；对每条 Generic 做“无品牌答案测试”：如果不写任何具体候选名称仍能完整回答，则淘汰；Branded 必须形成品牌适配或有边界比较。再查单意图、品牌边界、中性前提、译文和字段隔离。多子品类 Topic 按整批检查覆盖，单题可只落到一个产品词或子品类，不机械并列完整范围。
