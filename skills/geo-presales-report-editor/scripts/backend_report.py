...
CUSTOMER_BANNED = re.compile(
    ...
    r"正式结论|切片观察|题面已含品牌|题型机制|不能据此判断|"
...
VERIFICATION_SIGNAL_PATTERNS = {
    "visibility": re.compile(r"可见度|提及|平均提及排名"),
...
def module_purpose(module_id):
    return {
        ...
        "M07": "用后端已确认的匹配 Prompt 样本判断整体和主题的品牌提及与平均提及排名是否跨平台稳定，并保留样本与因果边界。",
...
def module_synthesis_rules(module_id):
    return {
        ...
        "M02": [
            "先判断是否获得提及；提及为零时平均提及排名写为短横线，只能判断缺席。",
            "已有提及时才讨论平均提及排名；该排名不是模型对品牌优劣的语义排序。",
            "提及、声量和平均提及排名只使用后端已确认的发现类问题样本；竞品类问题的比较结果单独表述胜者、平局和强度。",
        ...
        "M04": [
            "评价只使用 Sentiment 样本；如后端提供验证的 Attribute 诊断，按 Strength、Opportunity、Objection 区分，不把 target_attributes 直接写成已形成认知。客户文案统一称评价和验证，不混用品牌评价、功能核实等旧称。",
        ...
        "M07": [
            ...
            "分别判断提及是否一致和平均提及排名是否一致；所有平台均未提及时，不得讨论排名一致或竞争位置。",
    ...
    task = {
        ...
        "input": {
            ...
            "global_rules": [
                ...
                "发现与主要引用生态只使用 Discovery；竞品、验证和评价的问题本身可能提及目标品牌，不能证明品牌在用户未指定时进入回答。",
