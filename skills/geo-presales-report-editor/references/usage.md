...
2. 从附件、已确认统计文件或只读页面数据中提取证据；先拆发现类、竞品类、验证类、准确性类、评价类和品类认知类问题，再综合整体、主题、平台、引用与跨维度关系。
...
- M07 是否只解释后端匹配 Discovery Prompt 的正式一致性结果，没有本地拼样本、手算阈值或推断平台机制；客户文案统一用“主题”和“平均提及排名”。
